...
# ---------------------------------------------------------------------------
# Lazy singleton: load the full dataset once
# ---------------------------------------------------------------------------
...
def get_business_stats(business_id: str) -> dict:
    ...
    Returns:
        ...
        star_distribution.
        Returns a dict with review_count=0 if the business is not found.
    """
    df = _load_df()
    ...
    # Ensure all 5 star buckets are present (fill missing with 0)
...
if __name__ == "__main__":
    df = _load_df()
    # Pick a business that has at least 5 reviews
    sample_biz = (
    ...
    result = get_business_stats.invoke({"business_id": sample_biz})
    print("Business stats:")
    for k, v in result.items():
        print(f"  {k}: {v}")

    print("\nUnknown business:")
    print(get_business_stats.invoke({"business_id": "DOES_NOT_EXIST"}))
